...
import org.zerock.b4.dto.RequsetFileRemoveDTO;
...
public class UpDownController {
  ...
  public Map<String , String> removeFile(@PathVariable("fileName") String fileName){
    ...
    File orginFile = new File(uploadPath , fileName);

    try {
      String mimeType = Files.probeContentType(orginFile.toPath());

      if(mimeType.startsWith("image")){
        File thumbFile = new File(uploadPath, "s_"+fileName);
        thumbFile.delete(); //썸네일파일 삭제
      }
      orginFile.delete();
    ...
    // Map으로 반환 하는 이유는 Json 데이터 형식으로 나갔으면 하기에
  }
